...
func TestSocketDisconnect(t *testing.T) {
	...
	socket := socket.NewMock(socketURL)
...
func TestListenCommand(t *testing.T) {
	...
	command := socket.NewCommand(mockSocket, "Some.method", nil)
...
func TestListenCommandError(t *testing.T) {
	...
	command := socket.NewCommand(mockSocket, "Some.methodError", nil)
	resultChan := mockSocket.SendCommand(command)
	mockSocket.Conn().AddMockData(&socket.Response{
		ID:     mockSocket.CurCommandID(),
		Error:  &socket.Error{},
...
func TestListenCommandUnknown(t *testing.T) {
	...
	command := socket.NewCommand(mockSocket, "", nil)
	command.SetID(0)
	resultChan := mockSocket.SendCommand(command)
	mockSocket.Conn().AddMockData(&socket.Response{
		ID:     0,
